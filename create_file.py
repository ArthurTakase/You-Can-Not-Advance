...
async def create_files(message):
    ...
    try : #On regarde si les fichiers existent déjà pour le serveur
        ...
        #-------------------------------------------------------------------------------------
        #Anniversaire
        text_file = open("files\\"+str(id_serveur)+"\\birth.txt", "r", encoding="utf-8") #Test fichier Anniversaire
        text_file.close()
        #Autres fichiers ici
        return param[0]
        ...
        text_file = open("files\\"+str(id_serveur)+"\\birth.txt", "w+", encoding="utf-8") #Test fichier Anniversaire
        text_file.close()
        #Fichier rank
        text_file = open("files\\"+str(id_serveur)+"\\rank.txt","w+", encoding="utf-8")
